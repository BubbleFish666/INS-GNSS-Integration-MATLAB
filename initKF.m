...
KF.ba = [0; 0];  % accelorometer error (m/s^2) horizontal only
KF.bg = 0;  % gyro error (rad/s) yaw-axis noly
...
KF.P = zeros(8);
KF.P(1,1) = deg2rad(8)^2;  % yaw variance
KF.P(2:3,2:3) = 0.5^2 * eye(2);  % velocity variance (1.0 m/s)^2
KF.P(4,4) = (4.0 * llh_scale / meridionalRadius(INS.lat0 / llh_scale))^2;  % GNSS horizontal accuracy 2m -> lat (milli rad)^2
KF.P(5,5) = (4.0 * llh_scale / transverseRadius(INS.lat0 / llh_scale) / cos(INS.lat0 / llh_scale))^2;  % GNSS horizontal accuracy 2m -> lon (milli rad)^2
KF.P(6,6) = (0.03e-3 * 10)^2;  % accelorometer bias (0.03 mg)^2
KF.P(7,7) = (0.03e-3 * 10)^2;  % accelorometer bias (0.03 mg)^2
KF.P(8,8) = (deg2rad(10) / 3600)^2;  % gyro bias (8 deg/h)^2
...
KF.H = [zeros(2,1), eye(2), zeros(2), zeros(2,2), zeros(2,1);
        zeros(2,1), zeros(2), eye(2), zeros(2,2), zeros(2,1)];
...
% KF.Q(3, 3) = (2 * deg2rad(0.007))^2 * T;  % gyro noise 0.007 deg/s/sqr(Hz)
% KF.Q(4:5 , 4:5) = (5 * 120e-6 * 10)^2 * T * eye(2);  % accelorometer noise 120 ug/sqr(Hz)
% KF.Q(8:9, 8:9) = (5 * 0.04e-3 * 10)^2 * eye(2);  % accelerometer dynamic bias 0.04 mg
% KF.Q(13, 13) = (2 * deg2rad(10) / 3600)^2;  % gyro dynamic bias 10 deg/h

KF.Q(1,1) = (10 * deg2rad(0.007))^2 * T;  % gyro noise 0.007 deg/s/sqr(Hz)
KF.Q(2:3, 2:3) = (50 * 120e-6 * 10)^2 * T * eye(2);  % accelorometer noise 120 ug/sqr(Hz)
KF.Q(6:7, 6:7) = (50 * 0.04e-3 * 10)^2 * eye(2);  % accelerometer dynamic bias 0.04 mg
KF.Q(8,8) = (10 * deg2rad(10) / 3600)^2;  % gyro dynamic bias 10 deg/h
