...
% initial orientation
psi0 = 360 - 324.37;  % yaw at 26.0 s
% psi0 = 360 - 150.16;  % yaw at 67.0 s
% psi0 = 360 - 321.05;  % yaw at 114.0 s

R0 = R3(deg2rad(-psi0));
% R0 = R3(deg2rad(-35.63-59));
R_board_sensor = R3(deg2rad(-59));
...
LOG.v(1:range_end-range_start+1, 1:3) = nan;
...
while k <= range_end
    ...
    % f_ib_b = R0 * R_board_sensor * [faccx(k); faccy(k); faccz(k)];
    ...
    v = v_ + R0 * f_ib_b * T;
    % v = v_ + f_ib_b * T;
    v(3) = 0;  % disable velocity in Down direction
...
grid on
hold on
plot(t(data_range), LOG.v(:, 1))
plot(t(data_range), LOG.v(:, 2))
legend('vx', 'vy')
