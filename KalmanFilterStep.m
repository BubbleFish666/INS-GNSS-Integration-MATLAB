...
while k <= range_end
    ...
    % position
    F32n = [1 / (meridionalRadius(lat / llh_scale) + h), 0, 0;
            0, 1 / ((transverseRadius(lat / llh_scale) + h) * cos(lat / llh_scale)), 0;
            0, 0, -1];
    dllh = dllh + F32n * llh_scale * T * dv_eb_n_;
    ...
    LOG.KF.P3(k-range_start+1) = P(3, 3);
    LOG.KF.bg(k-range_start+1, :) = bg;
    ...
    LOG.KF.P3(k-range_start+1+1) = P(3, 3);
    LOG.KF.bg(k-range_start+1+1, :) = z(13:15);
